...
import Permission from "../schema-entities/Permission.schema";
import getMongooseModelForClass from "../db/index.db";

// Access to permissions collection

// CREATING DATA
export const create = async function (
    connection: Connection,
    documents: Permission | Permission[]
): Promise<FlattenMaps<Permission> | FlattenMaps<Permission>[]> {
    try {
        const model = getMongooseModelForClass<Permission>(Permission, connection);
...
export const find = async function (
    ...
    projection?: ProjectionType<Permission>,
...
export const findOne = async function (
    ...
    projection?: ProjectionType<Permission>,
    options?: QueryOptions<Permission>
): Promise<FlattenMaps<Permission> | null> {
    try {
        const model = getMongooseModelForClass<Permission>(Permission, connection);
...
export const findOneAndUpdate = async (
    connection: Connection,
    filter: FilterQuery<Permission>,
    update: UpdateQuery<Omit<Permission, "id" | "_id" | "updatedAt" | "createdAt">>,
    options?: QueryOptions<Permission>
): Promise<FlattenMaps<Permission> | undefined> => {
    try {
        const model = getMongooseModelForClass<Permission>(Permission, connection);
...
export const updateMany = async (
    ...
    update:
        | UpdateWithAggregationPipeline
        | UpdateQuery<Omit<Permission, "id" | "_id" | "updatedAt" | "createdAt">>
        | Omit<Permission, "id" | "_id" | "updatedAt" | "createdAt">,
